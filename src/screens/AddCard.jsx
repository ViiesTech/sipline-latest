...
const AddCard = ({ navigation }) => {
        ...
        if (date.length >= 2) {
            if (card.expiry.length < 2) {
                ...
                if (card.expiry.includes('/')) {
                    ...
                    setCard({ ...card, expiry: date.substring(0, 2) + '/' + date.substring(0, 2) });
                }
            }
        } else {
            setCard({ ...card, expiry: date });
    ...
    const isValid = () => {
        ...
        if (card?.owner?.length < 3) {
            Message('Name is not valid!', 'Name can only contains letters, minimum 3 letters are required.');
            return false;
        }
        if (validator.isEmpty(card?.expiry)) {
            Message('Card Expiry is Required!', 'Please enter your card expiry date.');
            return false;
        }
    ...
    return (
        <>
            <Background>
                <Wrapper>
                    <Header navigation={navigation} onlyTitle title={'Add Card'} withBack />
                    <Br space={2} />
                    ...
                    <Wrapper>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                            <View>
                                <Pera style={{ marginBottom: hp('0.5%'), paddingLeft: wp('1%') }}>Card Holder</Pera>
                                <Input
                                    value={card?.owner}
                                    placeholder="Name"
                            ...
                            <View>
                                <Pera style={{ marginBottom: hp('0.5%'), paddingLeft: wp('1%') }}>Card Number</Pera>
                                <Input
                                    styling={{ width: hp('19%') }}
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: hp('2%') }}>
                            <View>
                                <Pera style={{ marginBottom: hp('0.5%'), paddingLeft: wp('1%') }}>Card Expiry</Pera>
                                <Input
                                    styling={{ width: hp('19%') }}
                                    keyboardType="numeric"
                                    value={card?.expiry}
                            ...
                            <View>
                                <Pera style={{ marginBottom: hp('0.5%'), paddingLeft: wp('1%') }}>Card CVV</Pera>
                                <Input
                                    styling={{ width: hp('19%') }}
                                    value={card?.cvv}
                                    placeholder="CVV"
