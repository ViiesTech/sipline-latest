...
const SetProfilePic = ({ navigation, route }) => {
    ...
    const validation = () => {
        ...
        if (!validator.isAlpha(userInfo.name.replace(' ', '')) || userInfo.name?.length < 3) {
            Message('First Name is not valid!', 'Name can only contains letters, minimum 3 letters are required.');
            return false;
        }
    ...
    const ProfileImage = () => {
        return (
            <View style={{
                ...
                <Image
                    source={{ uri: userInfo.imageUrl ? userInfo.imageUrl : 'https://cdn.pixabay.com/photo/2012/04/26/19/43/profile-42914_640.png' }}
                    style={{
                        width: wp('30%'),
                        height: wp('30%'),
                        borderRadius: wp('100%'),
    ...
    return (
        <>
            <AuthLayout navigation={navigation}>
                <Wrapper>
                    <Br space={3} />
                    <H4 bold style={{ textAlign: 'center' }}>Set Profile Pic</H4>
                    <Br space={3} />
                    <ProfileImage />
                    <Br space={3} />
            ...
            <Btn
                loading={loader}
                onPress={handleSetProfile} style={{ position: 'absolute', zIndex: 1, bottom: hp('3%'), width: wp('88%'), alignSelf: 'center' }}>
                Next
            </Btn>
        </>
    );
