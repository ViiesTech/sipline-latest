...
export const handleLoading = (allStatesValue, naviagtion) => {
  return async dispatch => {
    ...
    try {
      ...
      naviagtion.replace('SetProfilePic', {
  ...
  return async dispatch => {
    ...
    try {
      const res = await axios.get(
        `${baseUrl}${endPoints.forgetPassword}?email=${allStatesValue}`,
      );
      dispatch({type: LOADING_STATE, payload: false});
      Message(res.data?.title, res.data?.message);
      naviagtion.navigate('OTP', {
        customerID: res.data?.data?.id,
        email: allStatesValue,
      });
    } catch (error) {
      errHandler(error);
      dispatch({type: LOADING_STATE, payload: false});
...
export const handleSignin = (
  ...
  return async dispatch => {
    ...
    try {
      ...
      naviagtion.replace(res.data?.data?.step_after_login, {
        customerID: res.data.data.customer_id,
      });
    } catch (error) {
      errHandler(error);
...
export const handleLoginPermissions = () => {
  return async dispatch => {
    ...
    dispatch({
      ...
      payload: {
        saveRemember: extractToken,
        localToken: extractToken2?.token,
        allUserData: extractToken2,
      },
    });
  };
};
